...
- 설정에서 켜면 **모델이 선택지를 띄운다.** 결정이 필요할 때 버튼으로 물어보고, 고른 것이 답으로 간다
...
ccdesk 는 그 도구를 직접 구현해 두고(`lib/mcp-ccdesk.js`, 의존성 없이 JSON-RPC 를 직접 말한다),
...
⚠️ **전체허용(`bypassPermissions`)과 같이 쓰면 아무 의미가 없다.** 승인이 필요한 순간 자체가
없으므로 카드가 뜰 일이 없다. 설정에서 그 조합을 고르면 화면이 알려준다.

### 모델이 선택지를 띄우게 하기

설정에서 **"결정이 필요하면 모델이 선택지를 띄우게 하기"** 를 켜면, 모델이 판단을 미뤄야 할 때
버튼이 있는 카드를 내민다. 고른 값이 곧바로 모델에게 돌아간다.

헤드리스 CLI 의 도구 목록에는 `AskUserQuestion` 이 **없다**(69개 중 없음, 2026-09-03 실측).
그래서 같은 일을 하는 도구를 ccdesk 가 직접 만들어 붙인다 — `mcp__ccdesk__ask_choice`.
답을 CLI 가 아니라 우리가 만들므로 동작이 확실하다.

승인과 **따로 켜고 끌 수 있다.** 승인은 꺼두고 선택지만 쓰는 것도 된다.
