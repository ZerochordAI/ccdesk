...
export class Run extends EventEmitter {
  ...
  #args() {
    ...
    // MCP 서버는 설정이 있으면 무조건 붙인다. 승인 창구는 그 위에 얹는 별개 선택이다.
    // 둘을 한 조건으로 묶으면 "승인은 끄고 선택지만" 이 안 된다 — 서버가 아예 안 붙는다.
    if (this.mcpConfigPath) a.push('--mcp-config', this.mcpConfigPath)
    if (this.permissionPromptTool) a.push('--permission-prompt-tool', this.permissionPromptTool)
    return a
  }
