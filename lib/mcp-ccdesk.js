 ...
 *
 * ⚠️ 여기서 한참 헤맸다. 가끔 "fetch failed" 가 났는데 서버는 멀쩡했다.
 *    Node 서버는 5초 놀면 유휴 소켓을 닫고(keepAliveTimeout), fetch 는 그 닫힌 소켓을
 *    재사용하려다 끊긴다. 승인은 몇 분씩 뜸해서 딱 이 구간에 걸린다.
 *    그래서 (1) 연결을 재사용하지 않게 하고 (2) 그래도 끊기면 한 번 더 시도한다.
 *    실패하면 거부로 떨어지므로, 여기서 조용히 지나가면 도구가 통째로 막힌다.
 */
async function ask(body) {
  if (!URL_BASE || !ASK_SECRET) throw new Error('ccdesk 주소를 모릅니다')
  const payload = JSON.stringify({ runId: RUN_ID, waitMs: WAIT_MS, ...body })
    ...
    try {
      const res = await fetch(URL_BASE + '/api/asks', {
        ...
        headers: {
          ...
          'x-ccdesk-ask': ASK_SECRET,
          // 소켓을 물려 쓰지 않는다. 재사용이 이 문제의 뿌리다.
          connection: 'close',
        },
        body: payload,
      })
      ...
      lastErr = e
      // 서버가 준 상태 코드면 다시 걸어도 같다. 연결 문제일 때만 다시 시도한다.
      if (!/fetch failed|ECONNRESET|socket hang up|other side closed/i.test(String(e && e.message))) throw e
