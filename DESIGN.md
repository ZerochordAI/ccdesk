...
### 9.2e ⚠️ plan 모드는 MCP 도구 호출을 막는다

`ask_choice` 를 모든 모드에서 켜두려 했는데 틀렸다. `plan` 에서는 안 된다.

```
plan 모드 도구 목록에 ask_choice: 있음      <- 목록에는 보인다
선택지 횟수: 0
거부: [{ tool_name: "mcp__ccdesk__ask_choice", ... }]
"Cannot call mcp__ccdesk__ask_choice while in plan mode"
```

**목록에 보이는 것과 부를 수 있는 것은 다르다.** 모델은 부르고, plan 이 막고, 거부로 끝난다.
켜봐야 아무 일도 안 일어나고 거부 기록만 쌓인다. 그래서 `plan` 에서는 선택지를 끈다.

교훈 하나 — "권한과 무관한 기능"이라고 **추론**했다가 틀렸다. 도구는 결국 도구라
권한 모드의 지배를 받는다. 재보기 전에는 무관하다고 단정하지 말 것.
