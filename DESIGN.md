...
> `HANDOFF.md` 를 먼저 읽을 것. 이 문서는 그 위에 얹는 **v1 의 고정 계약**이다.
...
| | 규칙 | 왜 |
|---|---|---|
| **R1** | 폴더명을 역해독하지 않는다. `cwd` 필드를 읽는다 | `...-myapp-web` 은 `myapp\web` 이었다 (HANDOFF 3.3) |
...
| **R9** | `permission_denied` 를 조용히 삼키지 않는다 | 헤드리스엔 승인 왕복이 없다 (HANDOFF 3.2) |
